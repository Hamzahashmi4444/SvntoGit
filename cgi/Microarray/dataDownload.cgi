...
sub main {
  ...
  #### Decide what action to take based on information so far
	my $pid = '';
 	if ($parameters{Get_Data} eq 'GET_AFFY_ARRAY_FILES') {
  	
		print_data_download_tab(ref_parameters => \%parameters);	#skip printing the headers since we will be piping out binary data and will use different Content headers
...
print qq~
<TR><TD></TD><TD COLSPAN="2"><B>Number Expression Conditions:  $n_condition_count</B></TD></TR>
<TR><TD></TD><TD><IMG SRC="$HTML_BASE_DIR/images/space.gif" WIDTH="20" HEIGHT="1"></TD></TR>
~;
########################################################################################
...
print qq~
<TR><TD></TD><TD COLSPAN="2"><B>Number Affy Chips:  $n_affy_chips</B></TD></TR>
<TR><TD></TD><TD><IMG SRC="$HTML_BASE_DIR/images/space.gif" WIDTH="20" HEIGHT="1"></TD></TR>
~;
########################################################################################
...
my $base_url = "$CGI_BASE_DIR/Microarray/dataDownload.cgi";
...
###Get some summary data for all the data types that can be downloaded	
 ...
 ##Check to see if there is some TWO_COLOR analysis data
  ...
  ##Set up the hash to control what sub tabs we might see
		...
		my $default_data_type = ( $parameters{type} eq '2color' ) ? 'TWO_COLOR' : 'AFFY';
		...
		display_sub_tabs(	display_type  	=> $display_type,
	                   ...
	                   ) if ( $n_two_color_runs && $n_affy_chips );
  ...
  ### Show the data that can be downloaded
		...
		if ($display_type eq 'AFFY'){
		  print $affy_info;
			...
			@diplay_files = qw(CEL XML RPT INFO JPEG EGRAM_PF.jpg EGRAM_T.jpg EGRAM_F.jpg);
  ...
  ## Print the data 
    ...
    # start the form to download affy files
  ...
  my $file_idx = $resultset_ref->{column_hash_ref}->{File_Root};
   ...
   ########################################################################################
    ...
    my $pad = '&nbsp;&nbsp;&nbsp;';
      ...
      my $value = $array_id . '__' . $display_file;
    ...
    if ( 1 ){
		push @{$resultset_ref->{column_list_ref}} , "$display_file";  #add on column header for each of the file types
...
sub check_for_file {
    ...
    if ( -r $file_path ) { 
      ...
      $log->error( "File: $file_path exists but is not readable\n" );
		  return 0;
    }
	}else{
