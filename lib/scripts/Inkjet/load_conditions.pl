...
## Presently, force module to be microarray and work_group to be Inkjet_admin
  if ($module ne 'Inkjet') {
      print "WARNING: Module was not Inkjet.  Resetting module to Inkjet\n";
      $work_group = "Inkjet_admin";
      $DATABASE = $DBPREFIX{$module};
  }
...
## If set_tag is set, get biosequences
  if ($set_tag) {
	  ...
	  FROM $TBIJ_BIOSEQUENCE BS
	  LEFT JOIN $TBIJ_BIOSEQUENCE_SET BSS ON (BSS.biosequence_set_id = BS.biosequence_set_id)
...
## Define local variables
	...
	$rowdata_ref = \%rowdata;
	$pk = $sbeams->updateOrInsertRow(table_name=>$TBIJ_CONDITION,
...
sub insertGeneExpression {
  ...
  foreach my $gene_expression_id (@rows){
    $sql = "DELETE FROM $TBIJ_GENE_EXPRESSION WHERE gene_expression_id='$gene_expression_id'";
  ...
  $sbeams->transferTable(source_file=>$source_file,
			 ...
			 table_name=>$TBIJ_GENE_EXPRESSION,
			 insert=>1,
			 verbose=>$VERBOSE,
			 testonly=>$TESTONLY,
			 );
  ...
  if ($set_tag && $id_hash_ref) {
	...
	$sbeams->updateOrInsertRow(table_name=>$TBIJ_GENE_EXPRESSION,
				   rowdata_ref=>$rowdata_ref,
				   return_PK=>0,
				   verbose=>$VERBOSE,
				   testonly=>$TESTONLY,
